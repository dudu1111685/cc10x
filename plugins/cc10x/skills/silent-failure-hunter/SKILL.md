...
This skill is the Codex-compatible form of the original Claude Code `silent-failure-hunter` specialist agent. Use it under `cc10x-router` orchestration or when explicitly asked to perform this specialist workflow role.

The original Claude Code agent metadata such as `tools`, `model`, `color`, and nested `skills` is preserved in the source file under `plugins/cc10x/agents/` for reference; Codex loads this file as a skill.

If the original instructions mention Claude Code-only primitives such as `TaskUpdate`, `TaskCreate`, `TaskList`, `TaskGet`, or `EnterPlanMode`, use the Codex runtime adapter from `cc10x-router`: preserve the same status, blocker, and Router Contract information in the final response and workflow artifact when the primitive is unavailable.
